import Link from "next/link";
import Image from "next/image";
import { ArrowUpRight } from "lucide-react";
import Reveal from "@/components/ui/Reveal";
import type { RelatedItem } from "@/components/ui/RelatedItemsSlider";

/**
 * Shared hero + card-grid section for the /rooms, /dining, /events listing
 * pages — same layout, only the copy, item mapping (Package → RelatedItem)
 * and per-category badges differ, which each thin page.tsx supplies.
 */
export default function CategoryListingSection({
  pillLabel,
  heading,
  description,
  basePath,
  items,
}: {
  pillLabel: string;
  heading: string;
  description: string;
  basePath: string;
  items: RelatedItem[];
}) {
  return (
    <section className="pt-28 md:pt-32 pb-16">
      <div className="max-w-[1200px] 2xl:max-w-[1440px] mx-auto px-4">
        <Reveal className="mb-10 px-2 text-center">
          <p className="bento-pill mx-auto w-fit mb-4">{pillLabel}</p>
          <h1 className="bento-title text-3xl md:text-5xl mb-3">{heading}</h1>
          <p className="text-bento-ink-soft text-sm max-w-lg mx-auto">{description}</p>
        </Reveal>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {items.map((item, i) => (
            <Reveal key={item.slug} delay={i * 80}>
              <Link href={`${basePath}/${item.slug}`} className="group block bento-card overflow-hidden h-full">
                <div className="relative h-56">
                  {item.image && (
                    <Image
                      src={item.image}
                      alt={item.title}
                      fill
                      className="object-cover transition-transform duration-700 ease-out group-hover:scale-105"
                      sizes="(min-width: 768px) 33vw, 100vw"
                    />
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent" />
                  {item.badge && <span className="absolute top-4 right-4 bento-pill-dark text-xs">{item.badge}</span>}
                </div>
                <div className="p-5 md:p-6">
                  <h2 className="bento-title text-lg mb-2">{item.title}</h2>
                  {(item.sub_title || item.description) && (
                    <p className="text-bento-ink-soft text-sm leading-relaxed mb-4 line-clamp-2">
                      {item.sub_title || item.description}
                    </p>
                  )}
                  {item.features && item.features.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-4">
                      {item.features.map((f) => (
                        <span key={f.label} className="flex items-center gap-1.5 bento-pill !py-1 !text-[0.6rem]">
                          {f.icon}
                          {f.label}
                        </span>
                      ))}
                    </div>
                  )}
                  <span className="bento-link">
                    {item.ctaLabel ?? "View"}
                    <ArrowUpRight size={14} />
                  </span>
                </div>
              </Link>
            </Reveal>
          ))}
        </div>
      </div>
    </section>
  );
}
