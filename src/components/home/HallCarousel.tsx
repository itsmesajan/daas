...
export default function HallCarousel({ venues }: { venues: Package[] }) {
  const [index, setIndex] = useState(0);
  const [paused, setPaused] = useState(false);
  const reduceMotion = useReducedMotion();
  const space = venues[index];
  const activeImage = toImageUrls(space.img)[0] ?? "";
              ...
              <AnimatePresence mode="sync">
                ...
                >
                  {activeImage && (
                    <Image
                      src={activeImage}
                      alt={`${space.title} at Hotel Daaas`}
                      fill
